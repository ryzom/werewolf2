...
#
# $Id: acinclude.m4,v 1.2 2006/07/24 15:28:11 mattr Exp $
# 
# =========================================================================
...
MY_NEL_HEADER_CHK([NeL Georges], [nel/georges/u_form.h], [NL_U_FORM_H], $nelgeorges_is_mandatory)
...
# =========================================================================
# AM_PATH_MYSQLPP : MYSQL++ checking macros

AC_DEFUN([AM_PATH_MYSQLPP],
[ is_mandatory="$1"

AC_REQUIRE_CPP()

# Get from the user option the path to the mysqlpp files location
AC_ARG_WITH( mysqlpp,
    [  --with-mysqlpp=<path>   path to the mysqlpp install files directory.
                          e.g. /usr/local])

AC_ARG_WITH( mysqlpp-include,
    [  --with-mysqlpp-include=<path>
                          path to the mysqlpp header files directory.
                          e.g. /usr/local/include])

AC_ARG_WITH( mysqlpp-lib,
    [  --with-mysqlpp-lib=<path>
                          path to the mysqlpp library files directory.
                          e.g. /usr/local/lib])

fmod_lib="mysqlpp"

if test $with_mysqlpp
then
    mysqlpp_includes="$with_mysqlpp/include"
    mysqlpp_libraries="$with_mysqlpp/lib"
fi

if test "$with_mysqlpp_include"
then
    mysqlpp_includes="$with_mysqlpp_include"
fi

if test "$with_mysqlpp_lib"
then
    mysqlpp_libraries="$with_mysqlpp_lib"
fi


# Set MYSQLPP_CFLAGS
if test "$mysqlpp_includes"
then
    MYSQLPP_CFLAGS="-I$mysqlpp_includes"
fi

# Set MYSQLPP_LIBS
if test "$mysqlpp_libraries"
then
    MYSQLPP_LIBS="-L$mysqlpp_libraries"
fi
MYSQLPP_LIBS="$MYSQLPP_LIBS -l$mysqlpp_lib"

_CPPFLAGS="$CPPFLAGS"
CPPFLAGS="$CXXFLAGS $MYSQLPP_CFLAGS"

AC_MSG_CHECKING(for MYSQLPP headers)
AC_EGREP_CPP( yo_mysqlpp,
[#include <mysql++.h>
#ifdef MYSQLPP_VERSION
   yo_mysqlpp
#endif],
  have_mysqlpp_headers="yes",
  have_mysqlpp_headers="no" )

if test "$have_mysqlpp_headers" = "yes"
then
    if test "$mysqlpp_includes"
    then
        AC_MSG_RESULT([$mysqlpp_includes])
    else
        AC_MSG_RESULT(yes)
    fi
else
...
# Test the libraries
AC_MSG_CHECKING(for MYSQLPP libraries)

CPPFLAGS="$CXXFLAGS $MYSQLPP_LIBS"

AC_TRY_LINK( , , have_mysqlpp_libraries="yes", have_mysqlpp_libraries="no")

CPPFLAGS="$_CPPFLAGS"

if test "$have_mysqlpp_libraries" = "yes"
then
    if test "$mysqlpp_libraries"
    then
        AC_MSG_RESULT([$mysqlpp_libraries])
    else
        AC_MSG_RESULT(yes)
    fi
else
...
mysqlpp_libraries="$mysqlpp_libraries"

if test "$have_mysqlpp_headers" = "yes" \
   && test "$have_mysqlpp_libraries" = "yes"
then
    have_mysqlpp="yes"
else
    have_mysqlpp="no"
...
if test "$have_mysqlpp" = "no" -a "$is_mandatory" = "yes"
then
    AC_MSG_ERROR([MYSQLPP is needed to compile Werewolf Services (http://tangentsoft.net/mysql++/).])
fi

...
#include "nvIO/nvMeshIO.h"
#include "nvIO/nvObjIO.h"
#include "nvIO/nvMeshPreprocessor.h"
...

{
	...
	unsigned int numTriangles = (unsigned int) loader->GetNumTriangles();
	if (numTriangles == 0)
		return false;
								  ...
								  std::vector<float>& vertexTexCoords)
{
	...
	numVertices = (unsigned int) (vpos.size()/3);
	...
	numVertices = (unsigned int) (outputAtts[ 0 ].floatVector_.size() / 3);

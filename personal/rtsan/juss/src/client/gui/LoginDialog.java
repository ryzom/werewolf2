...
public class LoginDialog {
	 ...
	 */
	public static HashMap init(Shell parent) {
		Display display = Display.getDefault();		
		LoginDialog thisClass = new LoginDialog();
		thisClass.createSShell(parent);
		MainWindow.getWindow().positionCenter(thisClass.sShell);
...
//		l_image.setBounds(new Rectangle(10,10,40,40));
	...
	private void onLogin() {
		...
		if(!t_userName.getText().equals("")) {
			settings.put("user",t_userName.getText());
		}
		else {
		...
		if(!t_password.getText().equals("")) {
			settings.put("password", t_password.getText());
		}
		else {
			MessageBox messageBox = new MessageBox(sShell, SWT.ICON_WARNING | SWT.OK);
			messageBox.setText("Field emty...");
